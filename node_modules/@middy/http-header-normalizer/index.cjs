...
const httpHeaderNormalizerMiddleware = (opts = {})=>{
    ...
    const httpHeaderNormalizerMiddlewareBefore = async (request)=>{
        ...
        if (request.event.multiValueHeaders) {
            const rawHeaders1 = {};
            const headers1 = {};
            for (const key1 of Object.keys(request.event.multiValueHeaders)){
                rawHeaders1[key1] = request.event.multiValueHeaders[key1];
                headers1[options.normalizeHeaderKey(key1, options.canonical)] = request.event.multiValueHeaders[key1];
            }
            request.event.multiValueHeaders = headers1;
            request.event.rawMultiValueHeaders = rawHeaders1;
        }
    };
    return {
